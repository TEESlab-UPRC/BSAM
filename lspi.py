...
class LSPI:
    ...
    def LSTDQ_online(self):
        """
        This is LSTDQ as per the lagoudakis paper modified to run "online",
        that is to update the policy with a few samples at a time
        It uses any up-to-now unused samples in order to update the policy weights.
        This is slower than the opt version
        """
        # Loop through the samples
        # for reverse, point would be to change the 'reward'!. reward what? being close to reality?????
        # how? reward getting actions close to the real????
        for sample in self.unused_samples:
            ...
            next_phi_diff = (phi - self.discount_factor*nextphi).T
            self.lstdq_B += numpy.dot(phi,next_phi_diff)
            self.lstdq_b += phi*sample.reward/self.reward_normalization_factor

        # find the new weights
        weights = numpy.dot(numpy.linalg.inv(self.lstdq_B),self.lstdq_b)
        return weights
    ...
    def create_poly_bak(self, s, n, t):
        """
        This function finds and returns the value of the variables of a function
        (used to represent a process), depending on an input vector representing a state.
        The type and degree of the function is specified with t,n.
        ...
        -4 Hermite A
        -5 Hermite B
        -6 Chebyshev 1st kind A
        -7 Chebyshev 1st kind B
        -8 Chebyshev 1st kind C
        -9 Chebyshev 2nd kind A
        -10 Chebyshev 2nd kind B

        Default in this lspi implementation is n=0~2; t=2; s.size = 1
        """
        c = []
        if numpy.size(s) == 1:
            x = [s]
        ...
        if t == 2:
            N = n
            g = numpy.zeros((numpy.size(x),N+1), 'f')
            d = 1
            for m in range(N+1):
                c.append((-1)**m/numpy.math.factorial(m)*numpy.math.factorial(n)/numpy.math.factorial(m)/numpy.math.factorial(n-m))
                g[:,m] = [i**m for i in x]
            ...
            # t=1 became second place since t=2 is the default in this lspi- perfrmance opt
            g = numpy.zeros((numpy.size(x),1), 'f')
            N = 0
            d = 1
            c.append(1)
            g[:,0] = [i**n for i in x]
            ...
            g = numpy.zeros((numpy.size(x),N+1), 'f')
            for m in range(N+1):
                c.append((-1)**m*(numpy.math.factorial(n)/numpy.math.factorial(m)/numpy.math.factorial(n-m))*(numpy.math.factorial(2*n-2*m)/numpy.math.factorial(n)/numpy.math.factorial(2*n-2*m-n)))
                g[:,m] = [i**(n-2*m) for i in x]
        elif t == 4:
            N = numpy.floor(n/2)
            d = numpy.math.factorial(n)
            g = numpy.zeros((numpy.size(x),N+1), 'f')
            for m in range(N+1):
                c.append((-1)**m/numpy.math.factorial(m)/numpy.math.factorial(n-2*m))
                g[:,m] = [(2*i)**(n-2*m) for i in x]
        elif t == 5:
            N = numpy.floor(n/2)
            d = numpy.math.factorial(n)
            g = numpy.zeros((numpy.size(x),N+1), 'f')
            ...
            if n == 0:
                ...
                g = numpy.zeros((numpy.size(x),N+1), 'f')
                for m in range(N+1):
                    c.append((-1)**m*numpy.math.factorial(n-m-1)/numpy.math.factorial(m)/numpy.math.factorial(n-2*m))
                    g[:,m] = [(2*i)**(n-2*m) for i in x]
        elif t == 7:
            if n == 0:
                d = 1
            ...
            g = numpy.zeros((numpy.size(x),N+1), 'f')
            for m in range(N+1):
                c.append((-1)**m*numpy.math.factorial(n-m)/numpy.math.factorial(m)/numpy.math.factorial(n-2*m))
                g[:,m] = [(2*i)**(n-2*m) for i in x]
        elif t == 10:
            N = numpy.floor(n/2)
            d = 1
